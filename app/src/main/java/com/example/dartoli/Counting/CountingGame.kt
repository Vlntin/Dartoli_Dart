...
class CountingGame(legs: Int, sets: Int, players: ArrayList<CountingPlayer>, needed_points: Int): Serializable, GameModel() {
        ...
        fun score_allowed(score: Int, amount: Int): Boolean{
                var actual_player = game_players[actualPlayerNumber]
                if (actual_player.points - score != 1 &&
                        actual_player.points > score){
        ...
        fun check_leg_finished(): Boolean {
                for (player in game_players){
                        if (player.points == 0){
                                ...
                                Log.d("less_darts", player.leg_thrown_darts.toString() + "," + player.throws_to_win.min().toString() )
                                reset_for_new_leg()
                                return true
                        }
                }
        ...
        fun finish_player_move(){
                if (actualDartsLeft == 1){
                        var actual_player = game_players[actualPlayerNumber]
                        actual_player.leg_average =(((needed_points.toDouble() - actual_player.points.toDouble()) / actual_player.leg_thrown_darts.toDouble()) * 3.0)
